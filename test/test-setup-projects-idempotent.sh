...
bash "$SCRIPT" >/dev/null || { echo "FAIL: first run errored"; exit 1; }
[[ -d "$PROJECTS_ROOT" ]] || { echo "FAIL: projects directory not created"; exit 1; }
snap1=$($HELPER "$PROJECTS_ROOT" 6)
bash "$SCRIPT" >/dev/null || { echo "FAIL: second run errored"; exit 1; }
snap2=$($HELPER "$PROJECTS_ROOT" 6)
if [[ "$snap1" != "$snap2" ]]; then
  echo "FAIL: projects setup not idempotent"; exit 1
fi
