...
# Setup temp home directory for safe testing
setup_temp_home() {
	HOME_DIR=$(mktemp -d)
	echo "🧪 Using temp home directory: $HOME_DIR"
	# Ensure cleanup
...
test_chezmoi_apply_dry_run() {
	...
	((TESTS_RUN++))
	# Test that dry-run command succeeds
	if chezmoi --no-pager --color=false apply --source "$PWD" --destination "$HOME_DIR" --dry-run --force; then
...
test_chezmoi_idempotence() {
	...
	first_run_output=$(chezmoi --no-pager --color=false apply --source "$PWD" --destination "$HOME_DIR" --verbose --force 2>&1)
	first_exit_code=$?
	...
	second_run_output=$(chezmoi --no-pager --color=false apply --source "$PWD" --destination "$HOME_DIR" --dry-run --force 2>&1)
	second_exit_code=$?
...
test_target_files_present() {
	...
	((TESTS_RUN++))
	local output
	output=$(chezmoi --no-pager --color=false apply --source "$PWD" --destination "$HOME_DIR" --dry-run --force 2>&1)
	local exit_code=$?
	...
	# Note: path will be relative or absolute depending on chezmoi version/config
	local missing_files=()
...
test_direnv_hooks_present() {
	...
	# direnv partials OR plugins which contain the required hooks.
	local issues=()

	# Check Zsh: either explicit partial or direnv plugin
	if ! grep -qE 'template\s+"(templates/)?partials/direnv_hook.tmpl"' "$PWD/dot_zshrc.tmpl" &&
		! grep -q "direnv" "$PWD/dot_zshrc.tmpl"; then
		issues+=("zsh direnv hook")
	fi

	# Check Bash: explicit partial
