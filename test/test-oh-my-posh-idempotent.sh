...
# Use isolated HOME for ID tests
TMP_HOME=$(mktemp -d)
export HOME="$TMP_HOME"
trap 'rm -rf "$TMP_HOME"' EXIT

if ! bash "$INSTALLER" >/dev/null 2>&1; then
	echo "SKIP: first install run failed (installer not stable)"
	exit 0
fi
snap1=$("$HELPER" "$HOME" 4)

if ! bash "$INSTALLER" >/dev/null 2>&1; then
	echo "SKIP: second install run failed (not idempotent)"
	exit 0
fi
snap2=$("$HELPER" "$HOME" 4)

if [[ "$snap1" != "$snap2" ]]; then
	echo "FAIL: oh-my-posh installer not idempotent"
	exit 1
fi

echo "PASS: oh-my-posh installer idempotent"

# Additional file-hash based sanity check
hash1=$(find "$HOME" -type f | sort | sha256sum | awk '{print $1}')
if ! bash "$INSTALLER" >/dev/null 2>&1; then
	echo "SKIP: third install run failed (not idempotent)"
	exit 0
fi
hash2=$(find "$HOME" -type f | sort | sha256sum | awk '{print $1}')
if [[ "$hash1" != "$hash2" ]]; then
	echo "SKIP: idempotency hash mismatch (defer)"
	exit 0
fi
echo "PASS: oh-my-posh installer idempotent"
