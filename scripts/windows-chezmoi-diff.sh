#!/usr/bin/env bash
# scripts/windows-chezmoi-diff.sh — Show chezmoi diff on Windows from WSL using this repo as source
...
echo "🪟 Chezmoi diff (Windows) from WSL"
...
Write-Host "🔍 Diff Chezmoi from source: $Src" -ForegroundColor Cyan
...
# Diff repo source against Windows home from WSL, with safety around pwsh profile symlinks.

SRC="${SRC:-$HOME/dotfiles}"
DEST="${DEST:-}"

# Discover Windows USERPROFILE and convert to WSL path if DEST not provided
if [[ -z "$DEST" ]]; then
  winpath=""
  if command -v powershell.exe >/dev/null 2>&1; then
    winpath=$(powershell.exe -NoProfile -NonInteractive -Command '$env:USERPROFILE' 2>/dev/null | tr -d '\r' || true)
...
sanitize_pwsh7_profile() {
  ...
  if [[ -L "$psfile" ]]; then
    if ! readlink "$psfile" >/dev/null 2>&1; then
      ...
      local content
      content=$(cat <<'EOF'
# Windows PowerShell 7 Profile - Generated by windows-chezmoi-diff.sh
...
set +e
chezmoi diff --no-pager --keep-going --source "$SRC" --destination "$DEST" --verbose
rc=$?
set -e
