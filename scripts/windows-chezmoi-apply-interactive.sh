...
# Interactive chezmoi apply to Windows home from WSL.
# - Disables pagers to avoid hangs
# - Leaves prompts enabled (no --force, no --no-tty)
...
sanitize_pwsh7_profile() {
	...
	if [[ -L "$psfile" ]]; then
		if ! readlink "$psfile" >/dev/null 2>&1; then
			local have_pwsh=0
			command -v pwsh.exe >/dev/null 2>&1 && have_pwsh=1
			local psbin="powershell.exe"
			...
			content=$(
				cat <<'EOF'
# Windows PowerShell 7 Profile - Generated by windows-chezmoi-apply-interactive.sh
...
# Avoid interactive pagers; keep prompts
export CHEZMOI_NO_PAGER=1
export PAGER="${PAGER:-cat}"
export GIT_PAGER=cat
export LESS=FRX

chezmoi apply --no-pager --source "$SRC" --destination "$DEST" --verbose
