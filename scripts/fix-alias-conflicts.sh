...
if grep -E '^alias ' "$DOTFILES_ROOT/.shell_common.sh" | sort | uniq -d | grep .; then
...
if grep -E '^([a-zA-Z0-9_]+)\(\) *{' "$DOTFILES_ROOT/.shell_functions.sh" | awk '{print $1}' | sort | uniq -d | grep .; then
