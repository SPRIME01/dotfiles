...
if [[ -L "$WIN_PROJECTS_LINK" ]] && [[ -d "$WIN_PROJECTS_LINK" ]]; then
    ...
    echo "💡 If you want to replace it with a symlink, you have two options:"
    echo "    1) Windows PowerShell (Developer Mode):"
    printf "       New-Item -ItemType SymbolicLink -Path \"C:\\Users\\%s\\projects\" -Target \"%s\"\n" "$WIN_USER" "$WSL_PROJECTS_WIN_PATH"
    echo "    2) From WSL (opens UAC prompt):"
    echo "       just force-projects-symlink"
