...
is_linux()   { detect_platform; [[ "${DOTFILES_PLATFORM}" == "linux" ]]; }
is_macos()   { detect_platform; [[ "${DOTFILES_PLATFORM}" == "macos" ]]; }
is_wsl()     { detect_platform; [[ "${DOTFILES_PLATFORM}" == "wsl" ]]; }
is_windows() { detect_platform; [[ "${DOTFILES_PLATFORM}" == "windows" ]]; }
is_unix()    { detect_platform; case "${DOTFILES_PLATFORM}" in linux|macos|wsl) return 0 ;; *) return 1 ;; esac }
